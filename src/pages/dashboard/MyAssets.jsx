...
function MyAssets() {
    ...
    return (
        ...
        <Card>
            <CardHeader variant="gradient" color="blue" className="mb-8 p-6">
                            <div className="flex items-center">
                                ...
                                <button onClick={() => setShowCreateModal(true)} className="bg-[#2F3D44] text-white py-2 px-4 rounded-md ml-2 hover:bg-[#379CF0] focus:outline-none">Request New Asset</button>
                                <button onClick={() => setShowCreateModal(true)} className="bg-[#2F3D44] text-white py-2 px-4 rounded-md ml-2 hover:bg-[#379CF0] focus:outline-none">Repair Request</button>
                            </div>
            </CardHeader>
            ...
            <table className="w-full min-w-[640px] table-auto">
                <thead>
                    <tr>
                        ...
                        <th><Typography variant="small" className="text-sm font-bold uppercase text-blue-gray-400 text-left">Code</Typography></th>
                ...
                <tbody>
                  {paginatedAssets.map((asset) => (
                        <tr key={asset.id} className="border-t">
                          ...
                          <td><Typography className="text-xs font-normal text-blue-gray-500">{asset.category_code}</Typography></td>
                          <td><Typography className="text-xs font-semibold text-blue-gray-600">{asset.category_name}</Typography></td>
                          <td><Typography className="text-xs font-semibold text-blue-gray-600">{asset.condition}</Typography></td>
                          <td><Typography className="text-center text-xs font-semibold text-blue-gray-600">{asset.quantity_in_stock}</Typography></td>
                          ...
                          <td><button onClick={() => handleViewClick(asset)} className="py-1 px-3 rounded-md mb-2 border-black"><VisibilityOutlinedIcon /></button>                                                      
                              <button className="py-1 px-3 rounded-md"><CreateOutlinedIcon  style={{ color: '#BC544B' }} /></button></td>
                        </tr>
                    ))}
                </tbody>
            </table>
        ...
        <MyAssetRepairModal showCreateModal={showCreateModal}
                setShowCreateModal={setShowCreateModal}
                handleCreateAsset={handleCreateAsset}
                departmentNames={departmentNames}
                newAsset={newAsset}
